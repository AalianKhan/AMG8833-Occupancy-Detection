...
void fillSurroundedPixels(float* object, float* filteredPixels) {
    ...
    do {
        ...
        // Use a static array to avoid repeated stack allocation
        static float updatedObject[64];
        memset(updatedObject, 0, sizeof(updatedObject)); // Clear the array
        ...
        for (int i = 0; i < 64; i++) {
            ...
            // If the pixel is surrounded by at least 2 object pixels, include it
            if (neighbors >= 2) {
        ...
        memcpy(object, updatedObject, sizeof(updatedObject));
        
    } while (changesMade && iteration < maxIterations); // Repeat until no changes are made or max iterations reached
}